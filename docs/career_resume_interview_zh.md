# PyAgentCLI 简历与面试学习路线

这份入口文档已经拆分成多个独立章节，方便导入 OCDN、Obsidian 或其他知识库。旧版长文不再作为主要阅读入口，后续请按下面的目录逐篇复习。

请从这里开始：

- [PyAgentCLI 学习路线总览](career_learning_path/00_index.md)
- [PyAgentCLI 学习路线 v2 设计稿](career_learning_path/v2_learning_route_design_zh.md)

核心章节：

- [01 先跑通 PyAgentCLI](career_learning_path/01_run_project.md)
- [02 ReAct 和 Tool Calling](career_learning_path/02_react_tool_calling.md)
- [03 Plan-and-Execute / DAG](career_learning_path/03_plan_execute_dag.md)
- [04 Memory 系统](career_learning_path/04_memory_system.md)
- [05 RAG 代码检索](career_learning_path/05_rag_code_retrieval.md)
- [06 Tool Call、HITL 和安全策略](career_learning_path/06_tool_hitl_safety.md)
- [07 Multi-Agent](career_learning_path/07_multi_agent.md)
- [08 Browser Tools 和联网搜索](career_learning_path/08_browser_search.md)
- [09 接入 MCP](career_learning_path/09_mcp_integration.md)
- [10 Prompt 分层和 Skill System](career_learning_path/10_prompt_skill_system.md)
- [11 多模型适配和 LLM Client](career_learning_path/11_multi_model_llm_client.md)
- [12 产品化：CLI UX、Git、Runtime API](career_learning_path/12_productization_cli_git_runtime.md)
- [旧版项目定位](career_learning_path/01_project_positioning.md)
- [旧版先把项目跑起来](career_learning_path/02_run_project.md)
- [03 简历篇：AI Agent 岗位写法](career_learning_path/03_resume_ai_agent.md)
- [04 简历篇：后端 / 平台 / 工具岗位写法](career_learning_path/04_resume_backend_platform.md)
- [05 面试篇：ReAct、Plan-and-Execute、Multi-Agent](career_learning_path/05_interview_react_plan_multi_agent.md)
- [06 面试篇：Memory、RAG、Context Engineering](career_learning_path/06_interview_memory_rag_context.md)
- [07 面试篇：Tool Call、HITL、MCP、Skill](career_learning_path/07_interview_tool_hitl_mcp_skill.md)
- [08 面试篇：Browser Tools、Eval Harness、Trace Eval](career_learning_path/08_interview_browser_eval.md)
- [09 开发复盘：真实问题和解决方式](career_learning_path/09_debug_pitfalls.md)
- [10 知识库沉淀卡片](career_learning_path/10_knowledge_base_cards.md)
- [11 一分钟项目介绍和追问模板](career_learning_path/11_pitch_and_followups.md)

这组文档参考 PaiCLI 学习路线的组织方式，但全部内容都改写为 PyAgentCLI 的原创项目材料，并补充了我们开发过程中真实遇到的问题。
